...
## Mean age (Days??)
```{r}
convert_pretty_print_table(si_mod_print, "mean_age") %>% 
  kable() %>% 
...
convert_pretty_print_table(si_mod_print, "agent_argument_type") %>% 
  mutate(
    Parameter = case_when(
    Parameter == "Agent Argument Type" ~ "Noun phrase type (Pronoun / Nount)", 
    TRUE ~ Parameter)
  ) %>% 
  kable() %>% 
  kable_styling(font_size = 12)
...
# Reported model results using dataset without imputed values {.tabset}
...
## Mean age (Days??)
```{r}
convert_pretty_print_table(si_mod_print_no_impute, "mean_age") %>% 
  kable() %>% 
  kable_styling(font_size = 12)
```
