...
# Post-hoc power analysis for Meta-analysis 

While power analysis is more suited for prospective analysis than for retrospective analysis, we conducted two simulations varying the true effect size to see if the null effects in the present meta-analysis can be attributed to the lack of power. Because there was no consensus on calculating power for the multilevel multivariate models we used in the meta-analysis, we conducted our simulations on a mixed effect model using R package MetaPower (Griffin, 2020). 
```{r}

ess <- seq(0.01, 1.5, 0.01)
...
In the first simulation, we varied the included experimental conditions on the x-axis. The estimated power of the meta-analysis was plotted along the y-axis. The three curves in different colors represent the difference in the estimated power, controlled for the true effect sizes and the heterogeneity in the data. The red dotted line denotes the estimated power for the current meta-analytic effect size (*d*  = 0.24). The power analysis suggests that the current estimate has moderate power (Power: `r round(mod_estimate$power[[2]], 2)`).

```{r}
#ess <- seq(0.01, 1.5, 0.01)

condition_n <- seq(10, 200, 1)

#condition_n<- seq(10, 12, 1)
es_ratio <- c(0.5, 1, 2)


es_sims <- lapply(es_ratio, function(y){
  
  lapply(condition_n, 
       function(x){
         mpower(
              effect_size = (null_model$beta %>% as.numeric()) * y,
              study_size = ma_data %>% select(n_1) %>% pull() %>% mean(),#questionable?
              k = x,
              i2 = i2/100,
              es_type = "d",
...
es_sims %>% 
  mutate(ES_ratio_print = case_when(
    ES_ratio == 0.5 ~ "x.5 Actual MA Effect Size", 
    ES_ratio == 1 ~ "Actual MA Effect Size", 
    ES_ratio == 2 ~ "x2 Actual MA Effect Size"
  )) %>% 
  ggplot(aes(x = condition, y = power, color = ES_ratio_print, group = ES_ratio)) + 
  geom_point()+
  geom_line() + 
  geom_vline(xintercept = nrow(ma_data), 
             color = "red", 
             linetype = "dotted") + 
  geom_hline(yintercept = mod_estimate$power[[2]], 
             color = "red", 
             linetype = "dotted") + 
    annotate("text", x = nrow(ma_data) + 50, y = mod_estimate$power[[2]] + .05, label = "Estimated Post-hoc Power", vjust = -0.5) + 
...
es_ratio <- c(0.5, 1, 2)
condition_n <- seq(10, 200, 2)
```


In the second simulation, we simulated a power analysis for the moderator analysis. Similar to the first simulation, we varied the number of conditions included in the experiment for three different possible effect sizes. More specifically, we used the effect size of intransitive conditions as baseline while varying the effect size of transitive conditions. The red dotted line denotes the estimated power for the current moderator analysis. The power analysis suggests that the current moderator analysis might be under-powered (Power: `r round(moderator_estimate$mod_power[[2]], 2)`).


```{r}


trans_es <-  intransitive_model$beta %>% as.numeric() * es_ratio



trans_es_sims <- lapply(es_ratio, function(y){
    lapply(condition_n, 
       function(x){
         mod_power(
              n_groups = 2,
              effect_sizes = c(transitive_model$beta %>% as.numeric() * y,
                               intransitive_model$beta %>% as.numeric()),
              study_size =  2*x,
              k = x,#they are equal 
              i2 = 0.75,# i2/100, the paper is recommending to use a value like this because this is an average / rule of thumb thing, pg31
...
trans_es_sims %>% 
  mutate(ES_ratio_print = case_when(
    es_ratio == 0.5 ~ "x.5 Actual Transitive Condition Effect Size", 
    es_ratio == 1 ~ "Actual Transitive Condition Effect Size", 
    es_ratio == 2 ~ "x2 Actual Transitive Condition Effect Size"
  )) %>% 
  ggplot(aes(x = condition, y = power, color = ES_ratio_print, group = ES_ratio_print)) + 
  geom_point()+
  geom_line() + 
  geom_vline(xintercept = n_study_transitive, 
             color = "red", 
             linetype = "dotted") + 
  geom_hline(yintercept = moderator_estimate$mod_power[[2]], 
             color = "red", 
             linetype = "dotted") + 
    annotate("text", x = n_study_transitive + 100, y = moderator_estimate$mod_power[[2]] + .05, label = "Estimated Post-hoc Power for Transitivity Effect", vjust = -0.5) + 
  xlab("Number of Conditions") +
  xlim(10, 200)+
  ylab("Estimated Power") + 
  guides(color=guide_legend(title=""))
