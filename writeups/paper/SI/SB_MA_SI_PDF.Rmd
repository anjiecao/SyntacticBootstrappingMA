...
This document was created from an R markdown file. The repository for the project can be found at XXXXXXXXXX. The data reported in the paper can be explored interactively at the [Metalab website](http://metalab.stanford.edu/).
...
Using these standardize  we calculated Cohen's *d* and the variances as follows (the implementation of the script can be found at XXXXXXXXXXXXXXXXXXXXXXXXXXX).
...
```{r fig.width=12.8, fig.height=9.5}
...
all_pair_counts <- list(ALL_CATEGORICAL_VARS,
  ...
  heatmaply(all_counts_wide_matrix,
            fontsize_row = 8,
            fontsize_col = 8, 
            key.title = "Number of Conditions")
```

## Ordered by groups        
```{r fig.width=12.5, fig.height=9.5}
...
visual_stimuli_plot <- ggplot(ma_data_with_predictions, aes(x = mean_age_months, y = d_calc)) +
...
power_plot <- qplot(ns, ps, geom = c("point","line"),
          data = pwrs) +
...
cowplot::plot_grid(visual_stimuli_plot, power_plot, labels = c("A", "B"))
```
...
ggplot(plotting_data, aes(x = transitive_es, y = estimated_power, color = k_ratio_print, group = k_ratio_print)) +
  ...
  annotate("text", x = .1, y = .83, label = "80% power") +
