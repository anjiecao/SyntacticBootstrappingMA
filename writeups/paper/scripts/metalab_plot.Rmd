...
age_data <- bind_rows(age_data, SB_DATA)
```



```{r}
summarize_age <- age_data %>% 
  group_by(dataset) %>% 
  summarise(
...
tidy_models_with_age <- left_join(all_models_tidy, 
                                  summarize_age, 
                                  by = "dataset_name") %>% 
  filter(model_type == "null") 

tidy_models_with_age %>% 
  ggplot(aes(x = mean_age_in_month, 
    ...
    coord_flip() +
    geom_point() +
    geom_text(aes(label = dataset_name), hjust = 0.5, vjust = -1.4, size = 3) +
