...
For testing phases, we coded the presence or absence of the practice phase and the character identification phase. Practice phase refers to the trials in which the children were prompted to identify a familiar action upon being prompted with a familiar verb. For example, the children may hear “Find jumping” and be expected to look or point at the events showing jumping).  `r num_practice_yes` conditions included at least one practice trial and `r num_practice_no` conditions did not. Character identification phase refers to trials in which the children were prompted to look for the actors in the testing events. For example, if at the test the children see a bunny and a duck engage in casual actions, they would be prompted “Where’s the bunny?” “Where’s the duck” during the character identification phase.  `r num_char_id_yes` conditions included at least one character identification trial, and `r num_char_id_no` conditions did not. 
...
```
The factor synchronicity, which refers to the alignment between the linguistic stimuli and the visual stimuli, has two levels: asynchronous or simultaneous. Some experimental conditions first presented the linguistic stimuli paired with irrelevant visual scenes (e.g. a person on the phone talking). The relevant visual stimuli were not shown until the training phase is over. For experimental conditions using this alignment, they were coded as "asynchronous"(*N* = `r num_video_asynchronous`). The procedure was coded as "simultaneous" if the very first training sentence was presented along with the visual stimuli depicting relevant action or along with an attention-getter or a blank screen, immediately followed by the relevant action  (*N* = `r num_video_simultaneous`). 
...
```
We also examined how the amount of learning experience provided during the experiment would contribute to the learning outcomes. Specifically, we coded the distribution of the training and the testing trials (Mass: *N* = `r num_mass`; Distributed: *N* = `r num_distributed`). A procedure was categorized as "mass" if and only if the infants were trained exclusively on one novel verb and tested on the same verb. We also coded how many times each novel verb was spoken in a syntactically-informative way ^[We also coded additional moderators, including the modality of the stimuli (video or animation), the types of actor in the stimuli (person or non-person), type of transitive event (direct causal action or indirect causal action), type of intransitive action (one-actor or pareallel-action), the number of train-test pair, the number of test trial per pair, the number of video repetitions. Their analyses can be found in the Supplementary information.]. 
...
```{r}
mod <- rma.mv(d_calc ~ character_identification + mean_age, V = d_var_calc,
                    random = ~ 1 | short_cite/same_infant/x_1,
                     method = "REML",
                     data = ma_data)
