...
<!-- the above method of calculating egger's test does not take into account the clustered structure of our data, as we do elsewhere. There's no way to add this structure using the regtest function; you'll need to do the manual regression method that I added to the markdown on publication bias to get the correct values. The red line on the funnel itself should also reflect the model estimate from the correct ma model. -->
