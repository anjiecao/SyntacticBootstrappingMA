...
SB_info <-  ml_dataset_info %>%
    filter(name == "Syntactic Bootstrapping")

SB_data <- metalabr::get_metalab_data(SB_info)
...
```
```{r}
SB_data %>% select(short_cite, unique_row) %>% distinct()
ma_data %>% select(short_cite, row_id) %>% distinct()
```
...
SB_data %>% select(short_cite, d_var_calc)
ma_data %>% mutate(row_id = as.character(row_id)) %>% select(short_cite, d_var_calc) 
```
