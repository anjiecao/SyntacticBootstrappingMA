...
To compare the effects of all theoretical moderators, we fit an additive model with all theoretical variables as fixed effects. We excluded vocabulary size from the additive model because it was highly correlated with age, and was only available for a subset of conditions (*N* = `r  num_vocabulary_available`). Figure \@ref(fig:megaPlot)a shows estimates for each of the single-predictor models along with the additive linear model. The additive model revealed estimates that were highly comparable to the single-predictor model.  
