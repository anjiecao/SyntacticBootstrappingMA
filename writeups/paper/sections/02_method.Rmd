...
For each paper, we entered metadata about the paper (e.g., citation), information to calculate effect sizes, and information about moderators. We entered a separate effect size for each experimental manipulation and age group per paper (we refer to these as "conditions"). Most papers therefore contained multiple conditions, corresponding to multiple effect sizes in our meta-analysis. Our final sample included `r n_effect_sizes` conditions. 
...
Using the raw coded data, we calculated an effect size estimate for each condition as Cohen's *d*.  Cohen's *d* was calculated as the difference between the proportion correct responses and chance (.5), divided by a pooled estimate of variance (see SI, Sec. 1 for example calculation). This is a standard practice in other similar meta-analyses that include a 2 Alternative Forced Choice (2AFC) paradigm. We decided to follow the standard practice with an additional consideration, which is that using a standardized baseline allowed us to examine the effect of intransitive sentences. Many original analyses compared the looking time at the causative action between the transitive conditions and intransitive conditions, effectively treating intransitive conditions as the baseline group. Although this approach could potentially mitigate children's baseline preference due to the perceptual saliency of the visual stimuli, there is a concerning possibility that the saliency effect is not additive to the linguistic stimuli the children receive. As a consequence, treating intransitive conditions as baseline groups could introduce more noises into the interpretations of looking time differences. In contrast, the within-subject, against-chance comparison enabled us to compare the strength of the effect in both transitive sentences and intransitive sentences directly, without introducing additional variances. Compared to the original between-subjects analysis, our analytic method offered a more conservative estimate of the effect sizes (See SI, Sec. 2 for direct comparison between two methods). Note that due to children's baseline preference, some experimental conditions would end up with negative effect sizes, which means that children looked longer at the incorrect action.
