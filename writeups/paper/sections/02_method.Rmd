...
Our final sample included  `r n_papers` individual papers, corresponding to `r n_effect_sizes` individual effect sizes and `r unique_infants` unique infants (Mean age: `r mean_age_day` days). <!-- convert this to a meaninful unit - months; days --->
...
<!-- if it's easy, it would be nice to walk through an example calculation of an effect size for a paricular paper/condition in the SI. Similiar to section 3 in https://mlewis.shinyapps.io/xtmem_SI/. --> 
...
<!-- I took out the motivation for coding these different variables, but I think we probably need a little bit. Can you add back in a very concise motivation for why we coded some of these moderator variables (particularly the ones that are not obvious (e.g. mass vs. distirbuted)) -->
