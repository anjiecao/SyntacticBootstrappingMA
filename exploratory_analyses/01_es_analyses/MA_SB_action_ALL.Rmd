...
```{r}
DATA_PATH <- "/Users/caoanjie/Desktop/Summer2020/SyntacticBootstrapping/data/processed/syntactic_bootstrapping_tidy_data.csv" # make all variables (i.e. things that might change) as capital letters at the top of the scripts
...
## Forest plot 
```{r}
ma_model <- rma.uni(ma_data$d_calc, ma_data$d_var_calc)
ma_model
forest(ma_model,
       header = T,
...
```
```{r}
ma_data %>% select(mean_age) %>% summarize(min = min(mean_age/30.44), max = max(mean_age/30.44))
```
...
```{r}
m_AAT <- rma.mv(d_calc ~  agent_argument_type2, V = d_var_calc,
                            random = ~ 1 | short_cite / unique_infant, data = ma_data)
