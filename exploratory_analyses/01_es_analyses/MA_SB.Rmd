...
```{r}
ma_data_old <- rma.mv(d_calc ~ mean_age, V = d_var_calc,
                            random = ~ 1 | short_cite, data = ma_data_old)
summary(ma_data_old)
```
...
SETUP_CATEGORICAL_VARS <- c("stimuli_actor", "character_identification", "practice_phase","test_mass_or_distributed","test_type","presentation_type")
...
CATEGORICAL_VARS_DICT[["presentation_type"]] <- c("immediate_after", "asynchronous")
...
ma_data_hmap <- ma_data %>% 
  filter(presentation_type != "simultaneous") 

ma_data_hmap

h <- create_ctable(CATEGORICAL_VARS_DICT, ma_data_hmap)
h
h %>% 
  pivot_wider(names_from = key_value, values_from = n) %>% 
  select(-name) %>% 
  rename(set_up_row = new_value) %>% 
  gather(set_up_colum, freq, 2:13) %>% 
...
h_heatmap_wider <- h_heatmap %>% 
  replace(is.na(.), 0) %>% 
  pivot_wider(names_from = set_up_colum, values_from = freq) 

h_heatmap_wider %>% 
  filter(set_up_row == "test_type action")


h_heatmap_wider <- as.data.frame(h_heatmap_wider)
rownames(h_heatmap_wider) <- c("character_identification no	",
                               "character_identification yes", 
                               "practice_phase no",
                               "practice_phase yes",
                               "presentation_type immediate_after",
                               "stimuli_actor non-person",
...
h_heatmap_wider %>% select(-set_up_row) %>%  heatmaply(keep.dendro = TRUE)
...
SETUP_CATEGORICAL_VARS <- c("stimuli_actor", "character_identification", "practice_phase","test_mass_or_distributed","test_type")
...
```{r}
dend <- hclust(dist(h_heatmap_wider, method = "euclidean"), method = "complete")
kable(cutree(dend, k = 4)) 
...
full <- rma.mv(d_calc, d_var_calc, mods = ~ mean_age + test_mass_or_distributed + practice_phase + character_identification + sentence_structure,
...
res <- glmulti(d_calc ~ mean_age + test_mass_or_distributed + practice_phase + character_identification + sentence_structure, data=ma_data,
