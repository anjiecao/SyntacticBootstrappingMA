...
## take in toccount sentence structure? 
```{r}
...
```{r, fig.height = 10}
CATEGORICAL_VARS <- c("sentence_structure", "agent_argument_type_clean", "patient_argument_type_clean", "stimuli_actor","agent_argument_number","transitive_event_type","inransitive_event_type",
...
ma_data_at %>% 
  ...
  ylab("Effect Size") +
  xlab("Age (days)") +
  ggtitle("Syntactical Bootstrapping effect size vs. Age (days), breakdown by agent argument") 
```
```{r}
m_age_method <- rma.mv(d_calc ~ mean_age + agent_argument_type_clean, V = d_var_calc,
...
```{r}
ma_data_young_only %>% 
  filter(agent_argument_type_clean != "noun_phrase") %>% 
  ggplot(aes(x = age_months, y = d_calc, size = n_1, color = agent_argument_type_clean)) +
  geom_point() +
  geom_smooth(method = "lm") +
  ylab("Effect Size") +
  xlab("Age (days)") +
  ggtitle("Syntactical Bootstrapping effect size vs. Age (days), breakdown by agent argument, young only") 
...
## age with visual transitive stimuli  {.tabset}
### all 
```{r}
ma_data %>% 
  ...
  ylab("Effect Size") +
  xlab("Age (days)") +
  ggtitle("Syntactical Bootstrapping effect size vs. Age (days), breakdown by transitive_event_type") 
```
...
ma_data_young_only %>% 
  ...
  ylab("Effect Size") +
  xlab("Age (months)") +
  ggtitle("Syntactical Bootstrapping effect size vs. Age (months), breakdown by transitive_event_type, young only") 
```

```{r}
m_data_young_transtivity <- ma_data_young_only %>% 
  filter(transitive_event_type != "minimal_contact") 

m_age_vs_tran_young <- rma.mv(d_calc ~ mean_age + transitive_event_type, V = d_var_calc,
                            random = ~ 1 | short_cite, data = m_data_young_transtivity)
...
ma_data_old %>% 
  ...
  ylab("Effect Size") +
  xlab("Age (months)") +
  ggtitle("Syntactical Bootstrapping effect size vs. Age (months), breakdown by transitive_event_type, old only") 
```
```{r}
m_data_old_transtivity <- ma_data_old %>% 
  filter(transitive_event_type != "minimal_contact") 

m_age_vs_tran_old <- rma.mv(d_calc ~ mean_age + transitive_event_type, V = d_var_calc,
                            random = ~ 1 | short_cite, data = m_data_old_transtivity)
